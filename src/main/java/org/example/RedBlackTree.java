...
public class RedBlackTree<K extends Comparable<K>> {
    ...
    public List<Integer> getRange(int low, int high) {
        ...
        getRangeHelper((Node<Integer>) root, low, high, range);
        return range;
    }

    private void getRangeHelper(Node<Integer> node, int low, int high, List<Integer> range) {
        if (node == null) return;

        if (node.key >= low) {
            getRangeHelper(node.left, low, high, range);
        }
        ...
        if (node.key <= high) {
            getRangeHelper(node.right, low, high, range);
        }
    }
}
